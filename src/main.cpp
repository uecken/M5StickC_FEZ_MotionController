...
/* ---Roll/Pitch/Yawの計算---*/
      ...
      //Serial.printf("%d %d %6.2f, %6.2f, %6.2f,%6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f  \r\n",loop_cnt,skillMotionCount,roll,pitch,yaw, accX, accY, accZ,gyroX,gyroY,gyroZ);
      //Serial.printf("%d %s %4.1f,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f \r\n",loop_cnt%10000,skillMode,roll,pitch,yaw, accX, accY, accZ,gyroX,gyroY,gyroZ);
      if(DEBUG)Serial.printf("%ld %s %d,%d,%d,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f \r\n",loop_cnt%10000,skillMode.c_str(),skillSwitch,G36Switch,viewSwitch,roll,pitch,yaw, accX, accY, accZ,gyroX,gyroY,gyroZ,unwrapYaw);
...
/* ---視点移動判定---*/ 
    if (loop_cnt%(Fs/txFreq)== 0 && viewSwitch==1 && M5.BtnB.isReleased()) {
    ...
    if(loop_cnt%(Fs/(txFreq/2))== 1 && Wire.available()){
      if(joystickMode=="view"){
      ...
      }else if(joystickMode=="move_wasd"){
        x_data = -1 * int(Wire.read());
        y_data = -1 * int(Wire.read());

        prev_JoyStick_vector = JoyStick_vector;
        if(x_data == 0) {}//Stick最右で255となる計算だが0となるため、0は処理しない}
        else if(abs(int(x_data)-120)>5 || abs(int(y_data)-120)>5){
          if(x_data < 100 && y_data < 100 ){//Stick左下
            JoyStick_vector = "left_down";
            if(prev_JoyStick_vector != JoyStick_vector){
               Keyboard.releaseAll();
               Keyboard.press(0x61);//a 
               Keyboard.press(0x73);//s
            }
          }else if(x_data < 100 && y_data > 150 ){//Stick左上
            JoyStick_vector = "left_up";
            if(prev_JoyStick_vector != JoyStick_vector){
              Keyboard.releaseAll();
              Keyboard.press(0x61);//a
              Keyboard.press(0x77);//w
            }
          }else if(x_data > 150 && y_data > 150){//Stick右上
            JoyStick_vector = "right_up";
            if(prev_JoyStick_vector != JoyStick_vector){
              Keyboard.releaseAll();
              Keyboard.press(0x77);//w
              Keyboard.press(0x64);//d
            }
          }else if(x_data > 150 && y_data < 100){//Stick右下
            JoyStick_vector = "right_down";
            if(prev_JoyStick_vector != JoyStick_vector){
              Keyboard.releaseAll();
              Keyboard.press(0x64);//d
              Keyboard.press(0x73);//s
            ...
            JoyStick_vector = "right";
            if(prev_JoyStick_vector != JoyStick_vector){
              Keyboard.releaseAll();
              Keyboard.press(0x64);//d
            } 
          }else if(x_data < 100){
            JoyStick_vector = "left";
            if(prev_JoyStick_vector != JoyStick_vector){
              Keyboard.releaseAll();
              Keyboard.press(0x61);//a
            }
          }else if(y_data < 100){
            JoyStick_vector = "down";
            if(prev_JoyStick_vector != JoyStick_vector){
              Keyboard.releaseAll();
              Keyboard.press(0x73);//s
            }
        ...
        button_data = Wire.read();
        if(int(button_data)>=1){ Keyboard.releaseAll(); }
        Serial.printf("x:%d y:%d button:%d\n", int(x_data), int(y_data), int(button_data));
      }
    }
